...
export default function RankingsPage() {
  ...
  const filtered = useMemo(() => {
    let list = players;
    if (mode === "watchlist") list = list.filter(p => watchlist.has(p.rank));
    if (mode === "draft" && showAvailable) list = list.filter(p => !drafted.has(p.rank));
    if (posFilter !== "ALL") {
      list = list.filter(p => posFilter === "DST" ? p.position === "DST" : p.position === posFilter);
    }
    if (search.trim()) {
  ...
  // ── Desktop sidebar (all modes) ──
  const desktopSidebar = (
    <div style={{ width: 260, flexShrink: 0, borderRight: "1px solid rgba(255,255,255,0.07)", overflowY: "auto", padding: "20px 16px 24px", display: "flex", flexDirection: "column", gap: 20, background: "#0D0F14" }}>
      {/* Logo + label */}
      <div>
        <MatchRivalLogo />
        <div style={{ fontSize: 10, fontWeight: 700, color: "#4B5268", letterSpacing: "0.08em", textTransform: "uppercase", marginTop: 6 }}>DRAFT RANKINGS</div>
      </div>

      {/* Mode nav */}
      <div>
        <div style={{ fontSize: 9, fontWeight: 700, color: "#4B5268", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>VIEWS</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
          {([
            { id: "rankings"   as Mode, label: "Rankings",    icon: "📊", color: "#3B82F6" },
            { id: "watchlist"  as Mode, label: "Watchlist",   icon: "⭐", color: "#F59E0B" },
            { id: "myRankings" as Mode, label: "My Rankings", icon: "★",  color: "#A78BFA" },
            { id: "draft"      as Mode, label: "Draft Mode",  icon: "⚡", color: "#22C55E" },
          ]).map(m => (
            <button key={m.id} onClick={() => setMode(m.id)} style={{
              display: "flex", alignItems: "center", gap: 10,
              padding: "9px 12px", borderRadius: 8, border: "none",
              background: mode === m.id ? `${m.color}18` : "transparent",
              color: mode === m.id ? m.color : "#8892AA",
              fontSize: 13, fontWeight: mode === m.id ? 700 : 500,
              cursor: "pointer", textAlign: "left",
              borderLeft: `3px solid ${mode === m.id ? m.color : "transparent"}`,
            }}>
      ...
      {/* Scoring format */}
      <div>
        <div style={{ fontSize: 9, fontWeight: 700, color: "#4B5268", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>SCORING FORMAT</div>
        <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, overflow: "hidden" }}>
          {(["PPR", "HALF", "STD"] as ScoringFormat[]).map(s => (
            <button key={s} onClick={() => setScoring(s)} style={{ flex: 1, fontSize: 10, fontWeight: 700, letterSpacing: "0.04em", padding: "6px 0", color: scoring === s ? "#fff" : "#4B5268", background: scoring === s ? "#3B82F6" : "transparent", border: "none", cursor: "pointer" }}>
              {s}
            </button>
          ))}
        </div>
      ...
      {/* Position filter */}
      <div>
        <div style={{ fontSize: 9, fontWeight: 700, color: "#4B5268", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>POSITION</div>
      ...
      {mode === "draft" && (
        <div>
          <div style={{ fontSize: 9, fontWeight: 700, color: "#4B5268", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>FILTER</div>
          <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, overflow: "hidden" }}>
            {["All Players", "Available"].map(opt => {
              const active = opt === "Available" ? showAvailable : !showAvailable;
              return (
                <button key={opt} onClick={() => setShowAvailable(opt === "Available")} style={{ flex: 1, fontSize: 10, fontWeight: 700, padding: "6px 0", color: active ? "#22C55E" : "#4B5268", background: active ? "rgba(34,197,94,0.18)" : "transparent", border: "none", cursor: "pointer" }}>
                  {opt}
                </button>
              );
            })}
          ...
          {drafted.size > 0 && (
            <div style={{ marginTop: 10, fontSize: 10, color: "#4B5268" }}>
              <span style={{ color: "#22C55E", fontWeight: 700 }}>{drafted.size}</span> drafted · <span style={{ color: "#8892AA", fontWeight: 700 }}>{350 - drafted.size}</span> available
            </div>
          )}
        </div>
      )}
      ...
      {mode === "myRankings" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <button style={{ width: "100%", fontSize: 11, fontWeight: 700, color: "#A78BFA", background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.28)", borderRadius: 8, padding: "8px", cursor: "pointer" }}>
            Save Rankings
          </button>
          {editedMoves > 0 && (
            <div style={{ fontSize: 10, color: "#4B5268" }}>
              <span style={{ color: "#A78BFA", fontWeight: 700 }}>{editedMoves}</span> moves made
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        <div style={headerStyle}>
          ...
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: "inline-flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, overflow: "hidden" }}>
              {["All Players", "Available"].map(opt => {
                const active = opt === "Available" ? showAvailable : !showAvailable;
                return (
                  <button key={opt} onClick={() => setShowAvailable(opt === "Available")} style={{ fontSize: 10.5, fontWeight: 700, padding: "5px 14px", color: active ? "#22C55E" : "#4B5268", background: active ? "rgba(34,197,94,0.18)" : "transparent", border: "none", cursor: "pointer" }}>
                    {opt}
                  </button>
                );
              })}
          ...
          <div style={{ display: "flex", gap: 4, overflowX: "auto", scrollbarWidth: "none" }}>
            {posFilters.map(f => (
              <button key={f.id} onClick={() => setPosFilter(f.id)} style={{ fontSize: 10, fontWeight: 700, padding: "5px 9px", borderRadius: 20, whiteSpace: "nowrap", cursor: "pointer", flexShrink: 0, border: "none", background: posFilter === f.id ? "rgba(59,130,246,0.18)" : "rgba(255,255,255,0.03)", color: posFilter === f.id ? "#3B82F6" : "#4B5268" }}>{f.label}</button>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        <div style={headerStyle}>
          ...
          <div style={{ display: "flex", gap: 4, overflowX: "auto", scrollbarWidth: "none", marginTop: 8 }}>
            {posFilters.map(f => (
              <button key={f.id} onClick={() => setPosFilter(f.id)} style={{ fontSize: 10, fontWeight: 700, padding: "5px 9px", borderRadius: 20, whiteSpace: "nowrap", cursor: "pointer", flexShrink: 0, border: "none", background: posFilter === f.id ? "rgba(139,92,246,0.18)" : "rgba(255,255,255,0.03)", color: posFilter === f.id ? "#A78BFA" : "#4B5268" }}>{f.label}</button>
            ))}
          </div>
        </div>
    ...
    return (
      <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: "#0D0F14" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          {/* Draft mode: my picks strip */}
          {mode === "draft" && myPickPlayers.length > 0 && (
            <div style={{ padding: "10px 16px", background: "rgba(34,197,94,0.04)", borderBottom: "1px solid rgba(34,197,94,0.12)", flexShrink: 0 }}>
              <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#22C55E", marginBottom: 7 }}>My Picks</div>
              <div style={{ display: "flex", gap: 6, overflowX: "auto" }}>
                {myPickPlayers.map(p => (
                  <div key={p.rank} style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(34,197,94,0.10)", border: "1px solid rgba(34,197,94,0.22)", borderRadius: 20, padding: "4px 8px 4px 5px", flexShrink: 0 }}>
          ...
          {/* Search bar (rankings / watchlist / myRankings) */}
          {mode !== "draft" && (
            <div style={{ padding: "14px 16px 10px", background: "#0D0F14", borderBottom: "1px solid #1A1F2E", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "8px 12px" }}>
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search players…" style={{ background: "none", border: "none", outline: "none", fontSize: 13, color: "#E8EBF4", fontWeight: 500, width: "100%", fontFamily: "inherit" }} />
              </div>
            </div>
          )}
  ...
  // ── Mobile default: Rankings / Watchlist ─────────────────────────────────────
      ...
      <div style={headerStyle}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 10, padding: "7px 10px", flex: 1, minWidth: 0 }}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><circle cx="11" cy="11" r="8" stroke="#4B5268" strokeWidth="2"/><path d="M21 21l-4.35-4.35" stroke="#4B5268" strokeWidth="2"/></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search players…" style={{ background: "none", border: "none", outline: "none", fontSize: 12, color: "#E8EBF4", fontWeight: 500, width: "100%", fontFamily: "inherit" }} />
          </div>
          <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, overflow: "hidden" }}>
            {(["PPR", "HALF", "STD"] as ScoringFormat[]).map(s => (
...
interface MyRankingsListProps {
  myRankingsPlayers: Player[];
  myRankings: number[];
  players: Player[];
        ...
        return (
          ...
          >
            ...
            <div style={{ width: 44, height: 44, borderRadius: "50%", overflow: "hidden", background: "#1C2133", border: `2px solid ${POS_RING[p.position] || "transparent"}`, flexShrink: 0, position: "relative" }}>
              <img src={`/headshots/${p.last_name}_${p.first_name}.jpg`} alt={p.name} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} onError={e => { (e.target as HTMLImageElement).src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg'/%3E"; }} />
            </div>
